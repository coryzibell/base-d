//! NEON-accelerated base256 encoding and decoding
//!
...
/// NEON-accelerated base256 encoding
///
...
pub fn encode(data: &[u8], dictionary: &Dictionary) -> Option<String> {
    ...
    // SAFETY: Caller verified NEON support (mandatory on aarch64)
    unsafe {
...
/// NEON-accelerated base256 decoding
pub fn decode(encoded: &str, dictionary: &Dictionary) -> Option<Vec<u8>> {
    ...
    // SAFETY: Caller verified NEON support (mandatory on aarch64)
    unsafe {
        if !decode_neon_impl_unicode(&chars, &reverse_map, &mut result) {
            return None;
        }
    }
...
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
unsafe fn decode_neon_impl_unicode(
    chars: &[char],
