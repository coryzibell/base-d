//! SIMD implementation for base256 (8-bit encoding) using NEON
//!
...
/// SIMD-accelerated base256 encoding using NEON
///
/// Processes 16 bytes at a time using NEON for memory operations.
/// Falls back to scalar for LUT lookups (256-entry LUT doesn't fit in SIMD registers).
#[cfg(target_arch = "aarch64")]
...
/// SIMD-accelerated base256 decoding using NEON
///
/// Processes chars and uses HashMap for reverse lookup.
#[cfg(target_arch = "aarch64")]
...
/// NEON base256 encoding implementation
///
/// Algorithm:
/// 1. Load 16 bytes with NEON
/// 2. Lookup each byte in 256-char LUT (scalar - LUT doesn't fit in registers)
/// 3. Store results
///
/// The LUT lookup dominates, but NEON loads ensure aligned memory access
...
unsafe fn encode_neon_impl(data: &[u8], lut: &[char; 256], result: &mut String) {
    ...
    for _ in 0..num_rounds {
        ...
        // Store to buffer for scalar lookup
        let mut input_buf = [0u8; 16];
        unsafe {
            vst1q_u8(input_buf.as_mut_ptr(), input_vec);
        }

        // LUT lookup and push (scalar - 256 entries don't fit in SIMD)
...
/// NEON base256 decoding implementation (Unicode version)
///
/// Algorithm:
